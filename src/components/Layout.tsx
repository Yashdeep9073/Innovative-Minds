...
import { Menu, X, Globe, Lock, ShieldCheck, LogOut, Search, Facebook, Twitter, Linkedin, Instagram, Phone, Mail } from 'lucide-react';
import { User } from '../types';
import { APP_VERSION } from '../config/version';
import { StandardizedSEO } from './StandardizedSEO';
...
const NavItem = ({ page, label, onNavigate, setIsMobileMenuOpen, currentPage }: { page: string; label: string; onNavigate: (p: string) => void; setIsMobileMenuOpen: (o: boolean) => void; currentPage: string }) => (
  <button
...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, onNavigate, onSearch, currentPage }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 font-inter">
      <StandardizedSEO 
        title={currentPage === 'landing' ? undefined : currentPage.charAt(0).toUpperCase() + currentPage.slice(1)}
      />
      <CookieConsent />
      <PWAInstallPrompt />
      <OfflineSync />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <a 
                href="https://www.facebook.com/1imist" 
                target="_blank" 
              ...
              <a 
                href="https://api.whatsapp.com/send?phone=260762523854&text=76MD4T" 
                target="_blank" 
                rel="noopener noreferrer" 
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white fixed inset-0 top-16 z-50 overflow-y-auto animate-in fade-in slide-in-from-right-10 duration-200 flex flex-col shadow-xl">
          <div className="px-4 pt-4 pb-20 space-y-2 flex flex-col">
            ...
            <NavItem page="landing" label="Home" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="all-workshops" label="Course Catalogue" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="free-tuitions" label="Free Tuitions" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="about" label="About Us" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="learners" label="Student Life" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="library" label="Digital Library" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="innovation" label="Innovation Hub" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
            <NavItem page="contact-us" label="Contact & Support" onNavigate={onNavigate} setIsMobileMenuOpen={setIsMobileMenuOpen} currentPage={currentPage} />
      ...
      <footer className="bg-gray-900 text-white pt-16 border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 mb-12">
            <div className="col-span-2 lg:col-span-2">
              ...
              <div className="flex gap-4">
                 <a href="https://www.facebook.com/1imist" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#D62828] transition-colors p-2" aria-label="Facebook"><Facebook size={20} /></a>
            ...
            <div className="col-span-1">
              ...
              <ul className="space-y-3 text-sm text-gray-400">
                ...
                <li onClick={() => onNavigate('free-tuitions')} className="cursor-pointer hover:text-white transition-colors py-1">Free Tuitions</li>
            ...
            <div className="col-span-1">
              <h4 className="font-bold mb-6 text-[#D62828]">Compliance</h4>
              <ul className="space-y-3 text-sm text-gray-400">
                <li onClick={() => onNavigate('privacy-policy')} className="cursor-pointer hover:text-white transition-colors py-1">Privacy Policy</li>
                <li onClick={() => onNavigate('terms-of-service')} className="cursor-pointer hover:text-white transition-colors py-1">Terms of Service</li>
                <li onClick={() => onNavigate('cookie-policy')} className="cursor-pointer hover:text-white transition-colors py-1">Cookie Policy</li>
                <li onClick={() => onNavigate('data-protection')} className="cursor-pointer hover:text-white transition-colors py-1">Data Protection</li>
              </ul>
            </div>
          ...
          <div className="border-t border-gray-800 py-8 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <div>
                <p className="text-xs text-gray-500">
                  © 2026 Innovative Minds Institute. All rights reserved.
                </p>
                <p className="text-[10px] text-gray-600 mt-1">
                  This site complies with all GDPR, SSL, and Global Digital Data Standards.
                </p>
              </div>

              <div className="mt-2 md:mt-0">
                <span className="px-3 py-1 text-[10px] font-semibold bg-gray-800 text-white rounded-full tracking-wide shadow-sm">
                  {APP_VERSION}
                </span>
              </div>
            </div>
