...
import { 
  ...
  Calendar, Target, Activity, ChevronRight,
  TrendingUp, Shield, Brain, Folder,
  MessageSquare, Sparkles, Layout, Bot, User, 
  Lightbulb, Send, MapPin, Flame, BarChart3, Zap, MoreHorizontal, ArrowRight
} from 'lucide-react';
import { ResponsiveContainer, Tooltip, AreaChart, Area, RadialBarChart, RadialBar, Legend } from 'recharts';
  ...
  return (
    <Card className="p-6 h-full overflow-hidden">
      ...
      <div className="relative flex items-center gap-4 overflow-x-auto pb-4 hide-scrollbar">
        ...
        {/* Nodes */}
        {enrollments.slice(0, 5).map((e, _i) => (
...
export const AcademicCalendar: React.FC<{ enrollments: Enrollment[] }> = ({ enrollments: _enrollments }) => {
  const events = [
...
export const MasteryRings: React.FC<{ enrollments: Enrollment[] }> = ({ enrollments: _enrollments }) => {
...
export const AnalyticsPulse: React.FC<{ enrollments: Enrollment[] }> = ({ enrollments: _enrollments }) => {
...
export const StudyPlannerWidget: React.FC<{ enrollments: Enrollment[] }> = ({ enrollments: _enrollments }) => {
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  const today = new Date().getDay() - 1; 
  const [loads] = useState(() => days.map(() => Math.random() * 80 + 10));
  return (
    <Card className="p-6 h-full flex flex-col bg-white border border-gray-100">
      ...
      <div className="flex justify-between items-end gap-2 h-32 mb-4">{days.map((day, i) => { const isToday = i === (today < 0 ? 6 : today); const load = loads[i]; return ( <div key={day} className="flex-1 flex flex-col justify-end items-center gap-2 group cursor-pointer"><div className={`w-full rounded-t-lg transition-all duration-500 relative ${isToday ? 'bg-[#D62828]' : 'bg-gray-100 group-hover:bg-gray-200'}`} style={{ height: `${load}%` }}>{isToday && <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-red-500 rounded-full animate-ping"></div>}</div><span className={`text-xs font-bold ${isToday ? 'text-[#D62828]' : 'text-gray-400'}`}>{day}</span></div> ) })}</div>
...
export const SuccessPredictorWidget: React.FC<{ progress: number }> = ({ progress }) => {
  const [randomFactor] = useState(() => Math.random() * 10);
  const probability = Math.min(99, Math.round(progress + 40 + randomFactor));
  return (
