...
import { Clock, X, Send, Sparkles, Linkedin, Award, Share2 } from 'lucide-react';
...
export const ApplicationStatusTracker: React.FC<TrackerProps> = ({ status, currentStep, courseTitle }) => {
  const [trackingId] = useState(() => Math.floor(Math.random()*10000));
  const steps = [
    { id: 1, label: 'Submitted', icon: Clock },
    { id: 2, label: 'Under Review', icon: SearchIcon },
    { id: 3, label: 'Decision', icon: Award },
    { id: 4, label: 'Enrolled', icon: SearchIcon } // Replaced Check with SearchIcon as Check was removed
  ];
    ...
    <Card className="p-6 mb-6 border-l-4 border-l-[#D62828] animate-in fade-in slide-in-from-top-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Tracking ID: IMI-{trackingId}</p>
        </div>
...
export const RecommendationCarousel: React.FC<{ onNavigate: (p:string)=>void }> = ({ onNavigate }) => {
