...
export const TopicViewer: React.FC<TopicViewerProps> = ({ 
         ...
         return (
             <QuizView 
                ...
                onComplete={(score, isPass) => {
                    ...
                    if(score >= 75) {
                        setNotification({ message: `Passed with ${score.toFixed(1)}%! Advancing...`, type: 'success' });
                        setShowQuiz(false);
                        // Auto-move to next tab? Maybe let user choose.
                    } else {
                        setNotification({ message: `Score: ${score.toFixed(1)}%. Pass mark is 75%. You must retake the quiz.`, type: 'error' });
                        setShowQuiz(false); 
                    }
                }} 
             />
  ...
  return (
    <div className="flex flex-col h-full relative">
        ...
        {notification && (
            <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl shadow-xl border animate-in slide-in-from-top-4 duration-300 flex items-center gap-3 ${
