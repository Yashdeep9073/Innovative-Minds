...
import { GovernmentsPage, InnovationHubPage, KidsPage, GovernmentDashboard } from './pages/InstitutionalPages';
import { CourseBuilder } from './pages/CourseBuilder';
import { CourseLandingPage } from './pages/CourseLandingPage';
import { PaymentPage } from './pages/PaymentPage'; 
import { EnrollmentWizard } from './pages/EnrollmentWizard';
import { CourseOrientationPage } from './pages/CourseOrientationPage'; 
import { UniversitiesPage as AllUniversitiesPage } from './pages/UniversitiesPage'; 
import { FreeTuitionsPage } from './pages/FreeTuitionsPage'; 
import { PrivacyPolicy, TermsOfService, CookiePolicy, DataProtectionStatement } from './pages/CompliancePages';
import { User } from './types';
import { auth, getUserProfile, testFirebaseConnection, onAuthStateChanged, getUserEnrollments } from './services/firebase';
...
function App() {
  ...
  const routeUserByRole = async (role: string, userId?: string) => {
      if (role === 'student' && userId) {
          try {
              const enrollments = await getUserEnrollments(userId);
              const activeCourse = enrollments.find(e => e.status === 'active' || e.status === 'in_progress');
              if (activeCourse) {
                  setCurrentWorkshopId(activeCourse.workshopId);
                  setCurrentPage('workshop-player');
                  return;
              }
          } catch (e) {
              console.error("Error checking active courses for redirect", e);
          }
      }
  ...
  useEffect(() => {
      ...
      if (firebaseUser) {
        ...
        if (userProfile) {
          ...
          if (!firebaseUser.isAnonymous && (currentPage === 'login' || currentPage === 'landing')) {
             await routeUserByRole(userProfile.role, userProfile.id);
          }
        } else {
           ...
           if (!firebaseUser.isAnonymous && (currentPage === 'login' || currentPage === 'landing')) {
               await routeUserByRole(fallbackUser.role, fallbackUser.id);
           }
        }
      } else {
        setUser(null);
    ...
    return () => unsubscribe();
  }, [currentPage]); // Added currentPage to deps to fix lint warning
  ...
  useEffect(() => {
    const pageTitles: Record<string, string> = {
        ...
        'payment': 'Payment - IMI',
        'universities': 'Universities - IMI',
        'free-tuitions': 'Free Tuitions - IMI',
        'privacy-policy': 'Privacy Policy - IMI',
        'terms-of-service': 'Terms of Service - IMI',
        'cookie-policy': 'Cookie Policy - IMI',
        'data-protection': 'Data Protection - IMI'
    };
  ...
  const handleLogin = async (userData: User) => {
    ...
    if (redirect) {
        ...
        await routeUserByRole(userData.role, userData.id);
    }
  };
  ...
  const renderContent = () => {
    switch (currentPage) {
      ...
      case 'universities': // New Route
        return <AllUniversitiesPage onNavigate={handleNavigate} />;
      case 'free-tuitions': // NEW ROUTE
        return <FreeTuitionsPage onNavigate={handleNavigate} />;
      case 'governments':
      ...
      case 'data-protection':
        return <DataProtectionStatement />;
      case 'security':
      case 'legal':
      case 'terms':
      case 'privacy':
