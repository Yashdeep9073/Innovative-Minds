...
// --- ACADEMIC PROGRAM ENRICHMENT (INSTITUTIONAL GRADE) ---

export const enrichAcademicProgram = async (
  title: string,
  level: string,
  outline: string
): Promise<{
  ...
  ai_suggested_career_paths: string[];
  ai_suggested_outcomes: string[];
} | null> => {
  const ai = getClient();
  if (!ai) return null;

  const SYSTEM_INSTRUCTION = `
    You are an expert Academic Accreditation Consultant.
    Your task is to enrich an academic program with institutional-grade metadata.
    
    SOURCES:
    - Use knowledge from O*NET, ISCED, and standard academic frameworks.
    - Ensure career paths are realistic and current.
    - Keywords should be relevant for SEO and academic search.
  ...
  const prompt = `
    Enrich the following academic program:
    Title: ${title}
    Level: ${level}
    Official Outline Summary: ${outline}

    Provide:
    1. A professional marketing description (approx 150 words) suitable for a university prospectus.
    2. A list of 10 academic/SEO keywords.
    3. A list of 5 concrete career pathways.
    4. A list of 5 measurable learning outcomes (Bloom's Taxonomy).

    Return JSON:
    {
      "ai_suggested_description": "...",
      "ai_suggested_keywords": ["...", "..."],
      "ai_suggested_career_paths": ["...", "..."],
      "ai_suggested_outcomes": ["...", "..."]
    }
  `;
  ...
  try {
    ...
    console.error("Program Enrichment Failed", e);
    return null;
  }
};
