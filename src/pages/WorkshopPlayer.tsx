...
import { 
  ...
  recordTopicProgress, payExamFee, submitFinalExam,
...
import { 
  ...
  LogOut, Loader2, Sparkles, WifiOff, Download, Zap, Settings
} from 'lucide-react';
...
export const WorkshopPlayer: React.FC<WorkshopPlayerProps> = ({ workshopId, onNavigate }) => {
  ...
  const toggleLowDataMode = () => {
    ...
    localStorage.setItem('low_data_mode', String(newState));
  };
  ...
  useEffect(() => {
    ...
    const loadClassroom = async () => {
      try {
        ...
        // Cache workshop for offline use
        await offlineStorage.saveContent(workshopId, 'course', w);
        ...
        if (currentUser) {
           ...
           if (enr) {
             ...
             // Cache enrollment for offline use
             await offlineStorage.saveContent(`enrollment_${workshopId}`, 'progress', enr);
           } else {
  ...
  const handleTopicComplete = () => {
      ...
      if (activeTopicIndex < currentEnrIndex) {
          ...
          if (nextIdx + 1 < totalTopics) {
            console.log('Preloading next module...', workshop.workshop_structure.topics[nextIdx + 1].title);
          }
      } else {
  ...
  const handleSectionUpdate = async (key: string, score: number) => {
      ...
      const updatedEnrollment = { ...enrollment, progressData: newProgressData };
      setEnrollment(updatedEnrollment);

      // Save to offline storage immediately
      await offlineStorage.saveContent(`enrollment_${workshopId}`, 'progress', updatedEnrollment);
      ...
      if (navigator.onLine) {
          try {
              ...
              // Queue for later sync if it failed despite being online
              await offlineStorage.addToSyncQueue('progress_update', { enrollmentId: enrollment.id, topicIndex: activeTopicIndex, key, score });
          }
      } else {
  ...
  const handleDownloadCourse = async () => {
    ...
    await downloadService.startCourseDownload(workshop);
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden font-inter">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 bg-white border-r transform transition-transform duration-300 md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
         ...
         <div className="p-4 border-t absolute bottom-0 w-full bg-white space-y-2">
            <div className="flex gap-2">
               <Button 
                 variant="outline" 
                 className="flex-1 justify-center text-[10px] px-1" 
                 onClick={toggleLowDataMode}
                 title={lowDataMode ? "Switch to Standard Mode" : "Switch to Low Data Mode"}
               >
                 <Zap size={12} className={lowDataMode ? "text-amber-500 fill-amber-500" : "text-gray-400"} />
      ...
      <main className="flex-1 overflow-y-auto relative">
         {isOffline && (
            <div className="bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-center justify-center gap-2 text-amber-700 text-xs font-medium">
              <WifiOff size={14} />
              <span>You are currently offline. Progress is being saved locally and will sync when you're back online.</span>
            </div>
          )}
         ...
         {viewMode === 'exam' && (
          ...
          {showDownloadManager && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <div className="w-full max-w-2xl h-[80vh] rounded-3xl overflow-hidden shadow-2xl">
