...
export const CourseLandingPage: React.FC<Props> = ({ courseId, onNavigate, user }) => {
  ...
  const handleRepair = async () => {
      ...
      // Since we removed on-demand generation, just show error
      alert("This course is currently unavailable. Please contact support.");
      setRepairing(false);
      setErrorState(true);
  ...
  if (errorState || !course) return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
          ...
          <p className="text-gray-600 max-w-md mb-8">
              We couldn't load the course data. Please try again or contact support.
          </p>
  ...
  return (
    <div className="font-inter bg-white relative">
      <SEO 
        title={`${course?.title || 'Course'} | Innovative Minds Institute`}
        description={course?.description || 'Enroll in this AI-powered course at Innovative Minds Institute.'}
        keywords={`${course?.title}, Online Course Africa, EdTech Zambia, Innovative Minds Institute`}
        url={`https://iminstitute.online/course/${courseId}`}
        schema={course ? {
          ...
          "provider": {
            ...
            "sameAs": "https://iminstitute.online"
          }
        } : undefined}
      />
         ...
         <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="text-center mb-12">
               <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-sm font-bold mb-4 border border-blue-100">
                  <Sparkles size={16} className="text-blue-500" /> Recommended Path
               </div>
