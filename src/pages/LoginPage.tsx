...
export const LoginPage: React.FC<LoginProps> = ({ onLogin, onNavigate }) => {
    ...
    try {
      ...
      if (mockUser) {
        ...
        try {
           ...
           throw new Error("Mock login requires Anonymous Authentication to be enabled in Firebase.");
        }
      ...
      if (email) {
        ...
        const pendingResult = await checkPendingEnrollment(result.user.uid);
        if (pendingResult && onNavigate) {
           onNavigate(`payment/${pendingResult.id}`);
        }
      } else {
      ...
      if (err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password') {
        msg = "Invalid Passkey or Email. If you don't have an account, please click 'Register'.";
      }
      if (err.code === 'auth/user-not-found') msg = "User not found. Please register.";
      setError(msg);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setStatusMsg("Google Login Successful! Checking enrollments...");
      await logLoginAttempt(result.user.email || result.user.uid, true, 'google');
      ...
      await logLoginAttempt('google_login_failed', false, 'google');
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {isRegistering ? (
              ...
              <div className="space-y-4 pt-4">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-3 px-4 bg-white border border-gray-300 rounded-xl shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#D62828] flex items-center justify-center gap-3 transition-colors"
                >
