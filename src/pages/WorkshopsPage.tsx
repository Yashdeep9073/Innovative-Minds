...
import { getUnifiedCatalog } from '../services/academicService';
...
export const WorkshopsPage: React.FC<{ onNavigate: (p: string) => void; initialSearchTerm?: string; initialCategory?: string }> = ({ onNavigate, initialSearchTerm = '', initialCategory = 'All' }) => {
  ...
  const [sortOption, setSortOption] = useState<'relevance' | 'newest' | 'trending'>('relevance');
  ...
  useEffect(() => {
    const fetchCourses = async () => {
        setLoading(true);
        try {
            const data = await getUnifiedCatalog();
            
            // Fallback for demo if empty
            if (data.length === 0) {
               // No longer generating mocks on the fly
               setWorkshops([]);
            } else {
               setWorkshops(data);
            }
  ...
  // 4. Catalog Logic: Grouping & Context Sorting
  const groupedCourses = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
       ...
       if (sortOption === 'trending') return 0.5 - Math.random(); 
    ...
    // Create "Recommended" (Simulated Recommendation Logic)
    ...
    return groups;
  }, [workshops, searchTerm, sortOption]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-inter">
      <SEO 
        title="Programs & Courses | Innovative Minds Institute"
        description="Explore our comprehensive catalog of AI-powered, accredited programs. From Digital Literacy to Master's Degrees, find your future at Africa's EdTech Hub."
        keywords="Online Courses Africa, Digital Literacy Zambia, AI Programs, Accredited Degrees, Innovative Minds Institute Courses"
        url="https://iminstitute.online/all-workshops"
      />
      ...
      <div className="bg-gray-900 text-white pt-16 pb-24 px-4 relative overflow-hidden">
         ...
         <div className="max-w-5xl mx-auto relative z-10 text-center">
            <h1 className="text-3xl md:text-5xl font-black mb-6">Find Your Future in Africa's Hub</h1>
      ...
      <div className="max-w-[1600px] mx-auto -mt-16 relative z-20">
         ...
         {loading ? (
            <div className="flex flex-col items-center justify-center h-64">
               ...
               <p className="text-gray-500 animate-pulse">Loading Catalogue...</p>
            </div>
         ) : (
