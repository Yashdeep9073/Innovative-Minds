...
import { User, Workshop, Enrollment } from '../types';
import { 
  subscribeToCollection, migrateStaticWorkshops, 
  db, bulkImportCourses, 
  auth, bootstrapAdmin, populateCourseContent, runAutoMigration,
  generateCatalogExpansion, getUserProfile
} from '../services/firebase';
import { 
  resetCatalog, 
  initializeOfficialCatalog, 
  populateCatalogContent 
} from '../services/catalogInitializationService';
import { getAdminUnifiedCatalog } from '../services/academicService';
import { collection, query, orderBy, getDocs, updateDoc, doc, serverTimestamp, writeBatch, limit } from 'firebase/firestore';
import { 
  Users, BookOpen, Database, PlusCircle, Activity, 
  UploadCloud, LayoutDashboard, GraduationCap, 
  CreditCard, TrendingUp, MessageSquare, Settings, Menu, X, Search, 
  RefreshCw, Lock, Unlock, Globe,
  ShieldCheck, MapPin, ListTodo, Megaphone, 
  Handshake, Brain,
  FileCheck, Leaf, Zap, Download, Trash2, ShieldAlert
} from 'lucide-react';
...
export const AdminDashboard: React.FC<{ onNavigate?: (p: string) => void }> = ({ onNavigate }) => {
  ...
  const [_populatingAll, _setPopulatingAll] = useState(false);
  const [resetting, setResetting] = useState(false);
  const [populatingId, setPopulatingId] = useState<string | null>(null);
  const [_importData, _setImportData] = useState('');
  const [_auditResults, _setAuditResults] = useState<{ active: number, completed: number, issues: number } | null>(null);
  ...
  useEffect(() => {
    const initAdmin = async () => {
       ...
       if (user) {
          if (user.email === 'admin@imlearn.org' || user.email?.endsWith('@imlearn.org') || user.email === 'maorderzambia@gmail.com') {
             await bootstrapAdmin(user.uid, user.email);
             setPermissionGranted(true);
          } else {
             // Check if they are an admin in the database
    ...
    initAdmin();
  }, []); // bootstrapAdmin is stable from firebase service

  useEffect(() => {
    if (!permissionGranted) return;
    const unsubUsers = subscribeToCollection<User>('users', [orderBy('createdAt', 'desc'), limit(50)], setUsers);
    const unsubEnrollments = subscribeToCollection<Enrollment>('enrollments', [orderBy('enrolledAt', 'desc'), limit(50)], setEnrollments);
    const unsubScholarships = subscribeToCollection<any>('scholarship_applications', [orderBy('submittedAt', 'desc'), limit(50)], setScholarships);
    const unsubTickets = subscribeToCollection<any>('complaints', [orderBy('timestamp', 'desc'), limit(50)], setTickets);
    const unsubLogs = subscribeToCollection<any>('admin_actions', [orderBy('timestamp', 'desc'), limit(50)], setSystemLogs);

    const fetchCourses = async () => {
       try {
         const data = await getAdminUnifiedCatalog();
         setCourses(data);
  ...
  }, [permissionGranted]); // subscribeToCollection and getAdminUnifiedCatalog are stable
  ...
  const handleNuclearMigration = async () => {
    if (!window.confirm("NUCLEAR ACTION: Run Full System Auto-Migration?\n\nThis will sync all static data, ensure academic levels, and repair catalog consistency.")) return;
    setSyncing(true);
    try {
      const result = await runAutoMigration();
      alert(`Success! Migrated ${result.migratedCount} workshops and synced ${result.levelsSynced} academic levels.`);
    } catch (e) {
      console.error(e);
  ...
  const handleCatalogExpansion = async () => {
    if (!window.confirm("NUCLEAR ACTION: Expand Catalog to 200 Courses?\n\nThis will generate skeletons for 200 academic programs across all levels.")) return;
    setExpanding(true);
    try {
      const created = await generateCatalogExpansion(200);
      alert(`Success! Created ${created} new course skeletons.`);
      // Refresh list
      const data = await getAdminUnifiedCatalog();
      setCourses(data);
    } catch (e) {
  ...
  const handlePopulateAll = async () => {
    if (!window.confirm("NUCLEAR ACTION: Populate ALL Course Content?\n\nThis will use AI to generate full academic content for EVERY course that is not yet ready. This may take a long time and consume significant API quota.")) return;
    setPopulatingAll(true);
    try {
      await populateCatalogContent();
      alert("Official content population complete.");
    } catch (e) {
      console.error(e);
      alert("Bulk population failed.");
    } finally {
      setPopulatingAll(false);
    }
  };

  const handleNuclearResetAndRebuild = async () => {
    const confirm1 = window.confirm("!!! EXTREME NUCLEAR ACTION !!!\n\nThis will DELETE ALL existing courses, workshops, and programs.\n\nAre you absolutely sure?");
    if (!confirm1) return;
    
    const confirm2 = window.confirm("FINAL WARNING: This action is destructive and cannot be undone. The catalog will be reset to the official 150-course portfolio.\n\nProceed?");
    if (!confirm2) return;
    ...
    try {
      ...
      // 2. Initialize
      if (auth.currentUser) {
        await initializeOfficialCatalog(auth.currentUser.uid);
      }
      
      alert("Catalog Reset & Official Initialization Successful! 150 courses created. You can now run 'Populate All Content' to generate academic material.");
      ...
      console.error(e);
      alert("Nuclear Reset & Rebuild failed.");
    } finally {
      setResetting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 font-inter">
        <div className="text-center bg-white p-8 rounded-2xl shadow-sm border border-gray-100 max-w-md">
          ...
          <Button onClick={() => { if (onNavigate) onNavigate('landing'); else window.location.href = '/'; }}>Return to Home</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 font-inter">
        <div className="text-center">
          <RefreshCw className="w-12 h-12 text-[#D62828] animate-spin mx-auto mb-4" />
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-inter flex relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto h-screen p-4 md:p-8 w-full">
         <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
               ...
               {activeSection === 'academic' && (
                  <>
                     <Button size="sm" onClick={() => onNavigate && onNavigate('program-manager')} icon={GraduationCap} className="w-full md:w-auto justify-center mt-2 md:mt-0">Program Manager</Button>
         ...
         {activeSection === 'system' && (
            <div className="space-y-8 animate-in fade-in">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="p-6 border-t-4 border-purple-600">
                     <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-purple-100 text-purple-600 rounded-lg"><Zap size={20}/></div>
                        <h3 className="font-bold">Nuclear Recovery</h3>
                     </div>
                     <p className="text-sm text-gray-500 mb-6">Repair system architecture, merge catalogs, and activate all LMS features.</p>
                     <div className="space-y-3">
                        <Button className="w-full justify-start" variant="outline" icon={RefreshCw} onClick={handleNuclearMigration} isLoading={syncing}>Run Auto-Migration</Button>
                        <Button className="w-full justify-start" variant="outline" icon={PlusCircle} onClick={handleCatalogExpansion} isLoading={expanding}>Expand to 200 Courses</Button>
                        <Button className="w-full justify-start" variant="primary" icon={Brain} onClick={handlePopulateAll} isLoading={populatingAll}>Populate All Content</Button>
                        <div className="pt-4 border-t border-gray-100">
                           <Button 
                             className="w-full justify-center bg-red-600 hover:bg-red-700 text-white" 
                             icon={Trash2} 
                             onClick={handleNuclearResetAndRebuild} 
                             isLoading={resetting}
                           >
                             Nuclear Reset & Rebuild
                           </Button>
                           <p className="text-[10px] text-red-500 mt-2 text-center font-bold uppercase">Destructive: Deletes all courses & Rebuilds Official 150 Catalog</p>
                        </div>
                     </div>
                  </Card>
                  ...
                  <Card className="p-6 border-t-4 border-blue-600">
                     ...
                     <p className="text-sm text-gray-500 mb-6">Manage Firestore security rules, permissions, and admin access.</p>
                     <div className="space-y-3">
                        <Button className="w-full justify-start" variant="outline" icon={Lock}>Verify Permissions</Button>
                        <Button className="w-full justify-start" variant="outline" icon={Users}>Manage Admins</Button>
                        <Button className="w-full justify-start" variant="outline" icon={FileCheck}>Audit Logs</Button>
                     </div>
                  </Card>
                  ...
                  <Card className="p-6 border-t-4 border-green-600">
                     ...
                     <p className="text-sm text-gray-500 mb-6">Backup, restore, and import/export institutional data.</p>
                     <div className="space-y-3">
                        <Button className="w-full justify-start" variant="outline" icon={Download}>Export Data</Button>
                        <Button className="w-full justify-start" variant="outline" icon={UploadCloud}>Import Data</Button>
                        <Button className="w-full justify-start" variant="outline" icon={Trash2} className="text-red-600 hover:bg-red-50">Clear Cache</Button>
                     </div>
                  </Card>
               </div>

               <Card className="p-6">
                  <h3 className="font-bold mb-4 flex items-center gap-2"><Activity size={18} className="text-green-600"/> System Activity Logs</h3>
                  <div className="space-y-3">
                     {systemLogs.slice(0, 10).map((log, i) => (
                        <div key={i} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg text-sm">
                           <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                           <span className="text-gray-400 w-24">{new Date(log.timestamp?.toDate ? log.timestamp.toDate() : log.timestamp).toLocaleTimeString()}</span>
                           <span className="font-medium text-gray-700">{log.action || log.event_type}</span>
                           <span className="text-gray-500 ml-auto">{log.courseTitle || log.details?.title || ''}</span>
                        </div>
                     ))}
                  </div>
               </Card>
         ...
         {(activeSection !== 'executive' && activeSection !== 'academic' && activeSection !== 'system') && (
