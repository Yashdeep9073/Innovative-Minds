...
export interface User {
  ...
  gpa?: number;
  programName?: string;
  universityProfile?: TutorRequest;
}
...
export interface University {
  ...
  logo: string;
  accredited: boolean;
  type: 'Public' | 'Private' | 'College' | 'Technical';
  location: string;
  description: string;
  faculties: {
    faculty: string;
...
// --- UNIFIED ACADEMIC CATALOG (PHASE 2 & 4) ---

export interface LearningCycle {
  introductory_notes: string; // 300-500 words
  video: VideoObject; // YouTube URL
  key_points: string[]; // 15 Key Lecture Points
  quiz: TopicQuiz; // 20 questions, 76% pass mark
  revision_points: string[]; // 15 Revision Points
}
...
export interface UnifiedTopic {
  ...
  type: 'mandatory' | 'elective';
  order: number;
  introductory_notes: string;
  
  // The 3 Learning Cycles (Mandatory for all levels)
  cycles: {
    cycle_1: LearningCycle;
    cycle_2: LearningCycle;
    cycle_3: LearningCycle;
  };
...
export interface UnifiedCourse {
  ...
  title: string;
  slug: string;
  level: CourseType; // Workshop, Certificate, Diploma, Degree, Masters
  ...
  // Structure
  topics: UnifiedTopic[];
  modules?: any[]; // For higher levels (Diploma, Degree, Masters)
...
export interface SourceReference {
  ...
  license: string;
  accessed_at: string;
  data_field: string; // Field this source enriched
}
...
export interface AcademicProgram {
  ...
  title: string;
  slug: string;
  official_outline: string;
  marketing_description: string;
  duration_months: number;
  credits: number;
  delivery_mode: 'online' | 'hybrid' | 'campus';
  ...
  // Archived AI Content (Institutional Audit)
  archived_ai_content?: {
...
export interface ProgramVersion {
  ...
  change_log: string;
  approved_by: string;
  archived_snapshot: AcademicProgram;
  approved_at: any;
}
...
export interface ProgramRelationship {
  source_id: string;
  target_id: string;
  relationship_type: 'progression_path' | 'prerequisite' | 'related';
}
